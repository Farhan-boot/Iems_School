    ...
    public class Webhook : IHttpHandler
    {
        ...
        private void ExecuteCommandSync(object command)
        {
            try
            {
                ...
                mFileHelper.WriteLog("git pull result:"+ result);
            }
            catch (Exception objException)
            {
                mFileHelper.WriteLog("git pull Exception:"+ objException.GetBaseException().ToString());
                // Log the exception
            }
        }
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            //Request.Headers["XYZComponent"] != null//X-Gitlab-Token
            var gitHookDiskPath = context.Server.MapPath("/Content/GitHook/");
            logFile = Path.Combine(gitHookDiskPath, "log.txt");
            batchFile = Path.Combine(gitHookDiskPath, "github.bat");

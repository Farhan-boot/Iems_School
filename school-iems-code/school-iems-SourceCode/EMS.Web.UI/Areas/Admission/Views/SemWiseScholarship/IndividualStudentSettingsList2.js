...
emsApp.controller('SemWiseScholarshipListCtrl', function ($scope, $http, $filter, $document, cfpLoadingBar, $sce) {
    ...
    $scope.individualStudentScholarshipAddModalOpen = function (semesterId) {
        $scope.StudentScholarship = angular.copy($scope.NewSemWiseScholarshipJson);

        $scope.StudentScholarship.EndSemesterId = semesterId;
        $scope.StudentScholarship.StartSemesterId = semesterId;
    ...
    $scope.showTrashedScholarshipList = function (isShowTrashedScholarship) {
        $scope.isShowTrashedScholarship = isShowTrashedScholarship;
        $scope.getSemWiseScholarshipList();
    };
        ...
        $http.get($scope.getPagedSemWiseScholarshipUrl, {
           ...
           , "studentId": $scope.SelectedStudentId
            ...
            if (!result.HasError) {
                ...
                $scope.ScholarshipSemesterIdList = result.DataExtra.ScholarshipSemesterIdList;
                log($scope.ScholarshipSemesterIdList);
    ...
    $scope.onChangeScholarshipParticularName = function (id) {
        ...
        var semesterId = $scope.StudentScholarship.StartSemesterId;
            ...
            if (yes) {
                ...
                $http.get($scope.getLockOrUnlockIndividualStudentScholarshipByIdUrl,
                    ...
                    if (!result.HasError) {
                        ...
                        /*if (isLock) {
                            $scope.getApplyOrCancelScholarshipForIndividualStudentByScholarshipId(row, true);
                        }*/
            ...
            success(function (result, status) {
                if (!result.HasError) {
                    ...
                    if (result.Data != null) {
                        ...
                        //$scope.getLockOrUnlockIndividualStudentScholarshipById($scope.StudentScholarship, true);
    ...
    $scope.getApplyOrCancelScholarshipForIndividualStudentByScholarshipId = function (row, isApply) {
        ...
        var msg = "Re-Check Eligibility & Active";
        if (!isApply) {
            msg = "Inactive";
        }
        bootbox.confirm("Are you sure you want to " + msg + " this data & Bill Recalculate?", function (yes) {
            if (yes) {
                log(isApply);
    ...
    $scope.selectAll = function () {
        ...
        for (var i = 0; i < $scope.nonAffectedScholarshipList.length; i++) {
            var entity = $scope.nonAffectedScholarshipList[i];

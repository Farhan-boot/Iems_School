...
emsApp.controller('UserLogCtrl', function ($scope, $http, $filter, $document, cfpLoadingBar, $sce) {
    ...
    $scope.DepartmentChangeAuditList = [];
    ...
    $scope.loadPage = function () {
        ...
        $scope.getDepartmentChangeAuditById();
    };
    ...
    $scope.getPagedLoginAuditById = function () {
        $http.get($scope.getPagedLoginAuditByIdUrl, {
            ...
            if (!result.HasError) {
                ...
                $scope.HasError = true;
                $scope.ErrorMsg = "Unable to get Student Login list data! " + result.Errors.toString();
                alertError($scope.ErrorMsg);
            }
        }).error(function (result, status) {
            $scope.HasError = true;
            $scope.ErrorMsg = "Unable to get Student Login list data! " + "Status: " + JSON.stringify(status).toString() + ". " + JSON.stringify(result).toString();
            alertError($scope.ErrorMsg);
        });
    };
    ...
    $scope.getDepartmentChangeAuditById = function () {
        $http.get($scope.getDepartmentChangeAuditByIdUrl, {
            params: {
                "userId": $scope.AccountId
            }
        }).success(function (result) {
            if (!result.HasError) {
                $scope.HasError = false;
                $scope.DepartmentChangeAuditList = result.Data;
            } else {
                $scope.HasError = true;
                $scope.ErrorMsg = "Unable to get Student Department Change list data! " + result.Errors.toString();
                alertError($scope.ErrorMsg);
            }
        }).error(function (result, status) {
            $scope.HasError = true;
            $scope.ErrorMsg = "Unable to get Student Department Change list data! " + "Status: " + JSON.stringify(status).toString() + ". " + JSON.stringify(result).toString();
            alertError($scope.ErrorMsg);
        });
    };
    ...
    , getPagedPasswordResetHistoryByIdUrl
        ...
        $scope.getDepartmentChangeAuditByIdUrl = getDepartmentChangeAuditByIdUrl;
    };

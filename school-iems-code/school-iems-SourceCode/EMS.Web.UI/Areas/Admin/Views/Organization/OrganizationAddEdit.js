...
emsApp.controller('OrganizationAddEditCtrl', function ($scope, $http, $filter, cfpLoadingBar) {
   ...
   $scope.loadPage= function (OrganizationId)
    {
       ...
       $scope.loadOrganizationExtraData($scope.OrganizationId);
   ...
   $scope.loadOrganizationExtraData= function(OrganizationId)
    {
        if(OrganizationId!=null)
            $scope.OrganizationId=OrganizationId;
            
            $http.get($scope.getOrganizationExtraDataUrl, {
                params: { "id": $scope.OrganizationId }
            }).success(function (result, status) {
                if (!result.HasError) {
                   $scope.HasError= false;
                      ...
                      if(result.DataExtra.StatusEnumList!=null)
                        ...
                        $scope.onAfterLoadOrganizationExtraData(result);
                } else {
                 $scope.HasError= true;
...
//======Custom Variabales goes hare=====
    ...
    $scope.Init = function (OrganizationId,getOrganizationByIdUrl,getOrganizationExtraDataUrl, saveOrganizationUrl,deleteOrganizationByIdUrl) {
        $scope.OrganizationId = OrganizationId;
        $scope.getOrganizationByIdUrl = getOrganizationByIdUrl;
        $scope.saveOrganizationUrl = saveOrganizationUrl;
        $scope.getOrganizationExtraDataUrl = getOrganizationExtraDataUrl;
  ...
  $scope.onAfterLoadOrganizationExtraData= function(result){
  ...
  //Child Table Bindings, need to fix
             ...
             $scope.OrganizationIdList =  result.DataExtra.OrganizationIdList; 
     */
    };

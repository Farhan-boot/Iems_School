...
emsApp.controller('DepartmentAddEditCtrl', function ($scope, $http, $filter, cfpLoadingBar) {
   ...
   $scope.loadPage= function (DepartmentId)
    {
       ...
       $scope.loadDepartmentDataExtra($scope.DepartmentId);
   ...
   $scope.loadDepartmentDataExtra= function(DepartmentId)
    {
        if(DepartmentId!=null)
            $scope.DepartmentId=DepartmentId;
            
            $http.get($scope.getDepartmentDataExtraUrl, {
                params: { "id": $scope.DepartmentId }
            }).success(function (result, status) {
                if (!result.HasError) {
                   $scope.HasError= false;
                      ...
                      if(result.DataExtra.StatusEnumList!=null)
                        ...
                        $scope.onAfterLoadDepartmentDataExtra(result);
                } else {
                 $scope.HasError= true;
...
//======Custom Variabales goes hare=====
    ...
    $scope.Init = function (DepartmentId,getDepartmentByIdUrl,getDepartmentDataExtraUrl, saveDepartmentUrl,deleteDepartmentByIdUrl) {
        $scope.DepartmentId = DepartmentId;
  ...
  $scope.onAfterLoadDepartmentDataExtra= function(result){

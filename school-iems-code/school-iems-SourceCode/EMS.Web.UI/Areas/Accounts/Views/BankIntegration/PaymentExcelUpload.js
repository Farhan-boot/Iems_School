...
emsApp.controller('BankPaymentExcelUploadCtrl', function ($scope, $http, $filter, Upload, $timeout, $sce) {
    ...
    $scope.IsEnableSaveButton = false;
    ...
    $scope.loadPage = function () {
        $scope.getBankPaymentExcelUploadDataExtra();
    };

    $scope.getBankPaymentExcelUploadDataExtra = function () {
        $scope.IsSuccessfullyCreditSaving = false;
        $http.get($scope.getBankPaymentExcelUploadDataExtraUrl)
    ...
    $scope.saveBankPaymentExcelData = function () {
        ...
        bootbox.confirm("Once you confirm, all amount will be permanently credited in the system for individual student and farther correction is very difficult process. Are you sure?",
            function(yes) {
                if (yes) {
        $scope.WaitForCreditSaving = true;
                    $http.post($scope.saveBankPaymentExcelDataUrl + "/", $scope.BankPaymentExcelTransactionJson).
            success(function (result, status) {
                if (!result.HasError) {
                    $scope.HasError = false;
    ...
    $scope.Init = function (uploadBankPaymentExcelFileUrl
                            , saveBankPaymentExcelDataUrl
                            , getBankPaymentExcelUploadDataExtraUrl) {

        $scope.uploadBankPaymentExcelFileUrl = uploadBankPaymentExcelFileUrl;
        $scope.saveBankPaymentExcelDataUrl = saveBankPaymentExcelDataUrl;
        $scope.getBankPaymentExcelUploadDataExtraUrl = getBankPaymentExcelUploadDataExtraUrl;
        $scope.loadPage();
    };
    ...
    $scope.uploadBankPaymentExcelFile = function () {
        //alert($scope.ExamId);
        if ($scope.myFile == null) {
            ...
            return;
                    ...
                    Upload.upload({
                        url: $scope.uploadBankPaymentExcelFileUrl,
                        data: {
                            file: $scope.myFile
                        }
                            ...
                            $timeout(function() {
                                ...
                                if (!$scope.result.HasError) {
                                    ...
                                    $scope.TotalCreditAmount = $scope.result.DataExtra.TotalCreditAmount;

...
emsApp.controller('ProfileEditCtrl', function ($scope, $http, $filter, cfpLoadingBar) {
    $scope.Student = [];
    $scope.StudentId = 0;
    $scope.ErrorMsg = "";
    ...
    $scope.getStudentById = function (StudentId) {
        ...
        $http.get($scope.getStudentByIdUrl, {
            ...
            if (!result.HasError) {
                ...
                $scope.IsDisabled = !$scope.Obj.Account.IsMigrate;
    ...
    $scope.saveStudent = function () {
        ...
        bootbox.confirm("Are you sure you want to confirm your registration? Once you confirm you cant undo.", function (yes) {
            if (yes) {
                $http.post($scope.saveStudentUrl + "/", $scope.Obj).
                    success(function (result, status) {
                        if (!result.HasError) {
                            ...
                            alertSuccess("You have successfully registered.");
                            $scope.Obj.Account.IsMigrate = false;
                            $scope.IsDisabled = true;
                            $scope.onAfterSaveStudent(result);
                        } else {
                            $scope.HasError = true;
                            $scope.ErrorMsg = result.Errors.toString() + " Unable to save!";
                            alertError($scope.ErrorMsg);
                        }
                    }).
                    error(function (result, status) {

...
angular.module("datetime").factory("datetime", ["$locale", function ($locale) {
    ...
    var SYS_TIMEZONE = (function () {
        var offset = -(new Date).getTimezoneOffset(),
			...
			min = absOffset % 60;
        //console.log(sign + num2str(hour, 2, 2) + num2str(min, 2, 2))
        ///return sign + num2str(hour, 2, 2) + num2str(min, 2, 2);//edited by pavel set sys timezone to zero
        return "+0000";
    })();
    ...
    function createNodes(format) {
        ...
        while ((match = tokenRE.exec(format))) {
            ...
            if (match.index > pos) {
                nodes.push(createNode("string", format.toString().substring(pos, match.index)));
                pos = match.index;
            }
        ...
        if (pos < format.length) {
            nodes.push(createNode("string", format.toString().substring(pos)));
        }
    ...
    function getInteger(str, pos) {
        ...
        str = str.toString().substring(pos);
    ...
    function setText(node, date, token, timezone) {
        date = new Date(moment(date).format("ddd MMM DD YYYY HH:mm:ss"));
        switch (token.name) {
            case "year":
    ...
    function deOffsetDate(date, timezone) {
        //date = new Date(moment(date).format("ddd MMM DD YYYY HH:mm:ss"));//added by pavel
        ...
        var parser = {
            parse: function (text) {
                ...
                if (parser.timezone) {
                    ...
                    //parser.model = new Date(date.getTime());//edited by pavel
                    parser.model = moment(new Date(date).toUTCString()).format("ddd MMM DD YYYY HH:mm:ss");// new Date(xxx.replace("GMT+0600", "GMT+0000"));//xxx
                }
            ...
            setDate: function (date) {
                ...
                date = moment(date).format("ddd MMM DD YYYY HH:mm:ss");//line added by pavel; Mon Aug 08 2016 00:57:18
                parser.model = new Date();
                //parser.timezone = "+0000";
                if (parser.timezone) {
                    parser.date = offsetDate(date, parser.timezone);
                } else {
                    parser.date = new Date(date); //new Date(Date.UTC(dat2.year(), dat2.getMonth(), dat2.getDate(), dat2.getHours(), dat2.getMinutes(), dat2.getSeconds()));
                }
    ...
    function linkFunc(scope, element, attrs, ngModel) {
        ...
        ngModel.$parsers.push(function (viewValue) {
            ...
            if (ngModel.$validate || validMinMax(parser.getDate())) {
                ...
                date = moment(new Date(date).toUTCString()).format("ddd MMM DD YYYY HH:mm:ss");//added by pavel
                //date = new Date(moment(date).format("ddd MMM DD YYYY HH:mm:ss"));//added by pavel
                if (modelParser) {
                    return modelParser.setDate(date).getText();
                } else {
                    // Create new date to make Angular notice the difference.
                    return date;//new Date(date.getTime());//edited by pavel
                }
            }

//fully cutomized code of original fuelux-wizard-custom.js included with beyond theme 
//By EMS Team
...
(function (b, c) {
    var a = function (f, e) {
        ...
        this.$prevBtn = $('[id=' + this.$element[0].id + '-actions]').find("button.btn-prev"); // edited by pavel $('#' + this.$element[0].id + '-actions').find("button.btn-prev");
        this.$nextBtn = $('[id=' + this.$element[0].id + '-actions]').find("button.btn-next");//edited by pavel $('#' + this.$element[0].id + '-actions').find("button.btn-next");
        /*End Customized to Enable Out Of Wizard Buttons*/
        //d = this.$nextBtn.children().detach();//by pavel
        ...
        setState: function () {
            ...
            if (h && h.last) {
                ...
                if (typeof this.lastText !== "undefined") {
                    ...
                    //var f = this.$nextBtn.children().detach();//by pavel
                    //this.$nextBtn.text(l).append(f)//by pavel
                }
            }

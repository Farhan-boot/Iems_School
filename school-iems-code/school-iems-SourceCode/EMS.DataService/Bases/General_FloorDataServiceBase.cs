//File: DataServiceBase for General_Floor
using System;
...
{
    public abstract class General_FloorDataServiceBase : BaseDataService
    {
        public General_FloorDataServiceBase(EmsDbContext emsDbContext, UserProfile usersProfile) : base(emsDbContext, usersProfile)
        {
        }
        ...
        #region Get By Id
        public General_Floor GetById(Int32 id)
        {
            return DbInstance.General_Floor.Find(id);
        }
        public General_Floor GetById(Int32 id, params string[] includeMembers)
        {
            IQueryable<General_Floor> query = (from q in DbInstance.General_Floor
                                              select q);
        ...
        #region Get List
        /// <summary>
        ///  to get all General_Floor set query parameter null.
        /// </summary>
        /// <param name="query">ex: IQueryable<General_Floor> query = DbInstance.General_Floor.Where(x => x.Test=="ABC");</param>
        /// <param name="pageSize"></param>
        /// <param name="pageNo"></param>
        /// <param name="count"></param>
        /// <param name="error"></param>
        /// <returns></returns>
        public List<General_Floor> GetPagedList(IQueryable<General_Floor> query, int? pageSize, int? pageNo, ref int count, out string error)
        {
            error = "";
            try
            {
                if (query == null)
                {
                    query = DbInstance.General_Floor;
                }
                count = query.Count();
        ...
        /// 
        /// </summary>
        /// <param name="query">ex: IQueryable<General_Floor> query = DbInstance.General_Floor.Where(x => x.Test==test);</param>
        /// <param name="error"></param>
        /// <returns></returns>
        public List<General_Floor> GetListByQuery(IQueryable<General_Floor> query, out string error)
        {
            try
            {
                error = "";
        ...
        public List<General_Floor> GetAll(out string error)
        {
            error = "";
            try
            {
                return DbInstance.General_Floor.ToList();
            }
            catch (Exception ex)
            {
                error = ex.ToString();
        ...
        public delegate bool SaveCallBackDelegate(General_Floor newObj, General_Floor unchangedDbObj, bool isNewObject, DbContextTransaction transactionScope, out string error, params string[] columnsToCopy);
        protected abstract bool HasSavePermission(out string error);
        
        public virtual bool IsValidToSave(General_Floor newObj, out string error)
        {
            error = "";
        ...
        /// <returns></returns>
        public bool Save(General_Floor newObj, SaveCallBackDelegate saveCallBackDelegate, out string error,  params string[] columnsToCopy)
        {
            error = string.Empty;
        ...
        /// <returns></returns>
        public bool Save(General_Floor newObj, out string error, DbContextTransaction scope = null, bool checkIsValidToSave = true, SaveCallBackDelegate saveCallBackDelegate = null, params string[] columnsToCopy)
        {
            error = string.Empty;
            General_Floor dbAddedObj =null;
        ...
        /// <param name="columnsToCopy">keep it empty to include all columns</param>
        /// <returns></returns>
        public bool Save(General_Floor newObj,ref General_Floor dbAddedObj, out string error,DbContextTransaction scope=null, bool checkIsValidToSave = true, SaveCallBackDelegate saveCallBackDelegate = null, params string[] columnsToCopy)
        {
            error = string.Empty;
            if (newObj == null)
            {
        ...
        /// <param name="error"></param>
        /// <returns></returns>
        public virtual bool IsValidToDelete(General_Floor objToDelete, out string error)
        {
            error = "";
            if (DbInstance.General_Room.Any(x => x.FloorId == objToDelete.Id))
            {
        ...
        public bool DeleteById(Int32 id, out string error, DbContextTransaction scope = null, bool checkIsValidToDelete = true)
        {
            ...
            try
            {
                General_Floor objToDelete = DbInstance.General_Floor.Find(id);
                if (objToDelete == null)
                {
                    return true;
                }
                ...
                DbInstance.General_Floor.Remove(objToDelete);
        ...
        public bool Delete(General_Floor objToDelete, out string error, DbContextTransaction scope = null, bool checkIsValidToDelete = true)
        {
            error = string.Empty;        
            if (objToDelete == null)
            {
            ...
            try
            {
                ...
                {
                    DbInstance.General_Floor.Attach(objToDelete);
                }
                DbInstance.General_Floor.Remove(objToDelete);
